...
public class DoubleHelper {
    static final /* synthetic */ boolean $assertionsDisabled = (!DoubleHelper.class.desiredAssertionStatus() ? true : $assertionsDisabled);
    ...
    static int exponent(long j) {
        return isDenormal(j) ? kDenormalExponent : ((int) (((kExponentMask & j) >>> kSignificandSize) & 4294967295L)) - 1075;
    }

    static boolean isDenormal(long j) {
        return (kExponentMask & j) == 0 ? true : $assertionsDisabled;
    }

    static boolean isInfinite(long j) {
        return ((j & kExponentMask) == kExponentMask && (kSignificandMask & j) == 0) ? true : $assertionsDisabled;
    }

    static boolean isNan(long j) {
        return ((j & kExponentMask) != kExponentMask || (kSignificandMask & j) == 0) ? $assertionsDisabled : true;
    }

    static boolean isSpecial(long j) {
        return (j & kExponentMask) == kExponentMask ? true : $assertionsDisabled;
    }
