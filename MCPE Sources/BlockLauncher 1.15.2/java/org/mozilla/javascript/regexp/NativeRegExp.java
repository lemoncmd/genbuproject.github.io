...
public class NativeRegExp extends IdScriptableObject implements Function {
    static final /* synthetic */ boolean $assertionsDisabled = (!NativeRegExp.class.desiredAssertionStatus() ? true : debug);
    ...
    private int lastIndexAttr = MAX_PROTOTYPE_ID;
    ...
    private static boolean backrefMatcher(REGlobalData rEGlobalData, int i, String str, int i2) {
        ...
        if ((rEGlobalData.regexp.flags & PREFIX) != 0) {
            for (int i3 = TEST; i3 < parensLength; i3 += MATCH) {
    ...
    private static boolean calculateBitmapSize(CompilerState compilerState, RENode rENode, char[] cArr, int i, int i2) {
        ...
        while (i4 != i2) {
            ...
            switch (cArr[i4]) {
                case Token.ASSIGN_BITOR /*92*/:
                    ...
                    switch (c) {
                        ...
                        case Token.DO /*119*/:
                            if (obj != null) {
                                reportError("msg.bad.range", BuildConfig.FLAVOR);
                                return debug;
                            }
            ...
            if ((compilerState.flags & PREFIX) != 0) {
                c2 = upcase((char) c);
                c = downcase((char) c);
                if (c2 < c) {
                    c2 = c;
    ...
    private static boolean classMatcher(REGlobalData rEGlobalData, RECharSet rECharSet, char c) {
        ...
        int i2 = c >> Id_toSource;
        if (!(rECharSet.length == 0 || c >= rECharSet.length || (rECharSet.bits[i2] & (MATCH << (c & 7))) == 0)) {
            i = TEST;
        }
    ...
    static RECompiled compileRE(Context context, String str, String str2, boolean z) {
        ...
        if (str2 != null) {
            i = TEST;
            for (int i3 = TEST; i3 < str2.length(); i3 += MATCH) {
        ...
        if (z && length > 0) {
            ...
            compilerState.result.chr = compilerState.cpbegin[TEST];
            compilerState.result.length = length;
            compilerState.result.flatIndex = TEST;
            compilerState.progLength += MAX_INSTANCE_ID;
        ...
        rECompiled.program = new byte[(compilerState.progLength + MATCH)];
        ...
        i = emitREBytecode(compilerState, rECompiled, TEST, compilerState.result);
        i2 = i + MATCH;
        rECompiled.program[i] = REOP_END;
        rECompiled.parenCount = compilerState.parenCount;
        switch (rECompiled.program[TEST]) {
            case PREFIX /*2*/:
                rECompiled.anchorCh = ANCHOR_BOL;
                break;
            case Token.LT /*14*/:
            case Token.GT /*16*/:
                rECompiled.anchorCh = rECompiled.source[getIndex(rECompiled.program, MATCH)];
                break;
            case Token.LE /*15*/:
            case Token.GE /*17*/:
                rECompiled.anchorCh = (char) (rECompiled.program[MATCH] & 255);
                break;
            case Token.LSH /*18*/:
            case Token.RSH /*19*/:
                rECompiled.anchorCh = (char) getIndex(rECompiled.program, MATCH);
                break;
            case Token.DELPROP /*31*/:
                RENode rENode = compilerState.result;
                if (rENode.kid.op == REOP_BOL && rENode.kid2.op == REOP_BOL) {
                    rECompiled.anchorCh = ANCHOR_BOL;
                    break;
                }
        }
    ...
    private static void doFlat(CompilerState compilerState, char c) {
        ...
        compilerState.result.length = MATCH;
        compilerState.result.flatIndex = -1;
        compilerState.progLength += Id_toSource;
    }
        ...
        while (rENode != null) {
            ...
            switch (rENode.op) {
                ...
                case Token.LT /*14*/:
                    ...
                    if (rENode.flatIndex == -1 || rENode.length <= MATCH) {
                        if (rENode.chr >= '\u0100') {
                            if ((compilerState.flags & PREFIX) != 0) {
                ...
                case Token.MOD /*25*/:
                    if (rENode.min == 0 && rENode.max == -1) {
                        bArr[i3 - 1] = rENode.greedy ? REOP_STAR : REOP_MINIMALSTAR;
                    } else if (rENode.min == 0 && rENode.max == MATCH) {
                        bArr[i3 - 1] = rENode.greedy ? REOP_OPT : REOP_MINIMALOPT;
                    } else if (rENode.min == MATCH && rENode.max == -1) {
                        bArr[i3 - 1] = rENode.greedy ? REOP_PLUS : REOP_MINIMALPLUS;
                    } else {
                        if (!rENode.greedy) {
                            bArr[i3 - 1] = REOP_MINIMALQUANT;
                        }
                        i3 = addIndex(bArr, addIndex(bArr, i3, rENode.min), rENode.max + MATCH);
                    }
                    i2 = addIndex(bArr, addIndex(bArr, i3, rENode.parenCount), rENode.parenIndex);
                    emitREBytecode = emitREBytecode(compilerState, rECompiled, i2 + PREFIX, rENode.kid);
                    i3 = emitREBytecode + MATCH;
                    bArr[emitREBytecode] = REOP_ENDCHILD;
                    resolveForwardJump(bArr, i2, i3);
                    continue;
                case Token.NEG /*29*/:
                    i2 = emitREBytecode(compilerState, rECompiled, addIndex(bArr, i3, rENode.parenIndex), rENode.kid);
                    i3 = i2 + MATCH;
                    bArr[i2] = REOP_RPAREN;
                    i3 = addIndex(bArr, i3, rENode.parenIndex);
                    continue;
                ...
                case Token.STRING /*41*/:
                    emitREBytecode = emitREBytecode(compilerState, rECompiled, i3 + PREFIX, rENode.kid);
                    i2 = emitREBytecode + MATCH;
                ...
                case Token.LOCAL_LOAD /*54*/:
                case Token.GETVAR /*55*/:
                    Object obj = rENode.op == REOP_ALTPREREQi ? MATCH : TEST;
                    addIndex(bArr, i3, obj != null ? upcase(rENode.chr) : rENode.chr);
                    i3 += PREFIX;
                    addIndex(bArr, i3, obj != null ? upcase((char) rENode.index) : rENode.index);
                    i2 = i3 + PREFIX;
                    break;
                default:
                    break;
            ...
            emitREBytecode = emitREBytecode(compilerState, rECompiled, i2 + PREFIX, rENode.kid);
            int i4 = emitREBytecode + MATCH;
            bArr[emitREBytecode] = REOP_JUMP;
            emitREBytecode = i4 + PREFIX;
            resolveForwardJump(bArr, i2, emitREBytecode);
            i2 = emitREBytecode(compilerState, rECompiled, emitREBytecode, rENode2);
            emitREBytecode = i2 + MATCH;
            bArr[i2] = REOP_JUMP;
            i3 = emitREBytecode + PREFIX;
            resolveForwardJump(bArr, i4, i3);
            resolveForwardJump(bArr, emitREBytecode, i3);
            rENode = rENode.next;
            i = i3;
        }
        return i;
    ...
    private static String escapeRegExp(Object obj) {
        ...
        while (indexOf > -1) {
            ...
            indexOf = scriptRuntime.indexOf(47, indexOf + MATCH);
            i3 = i2;
            stringBuilder = stringBuilder2;
            i = i3;
        }
    ...
    private Object execSub(Context context, Scriptable scriptable, Object[] objArr, int i) {
        ...
        if (objArr.length == 0) {
            ...
            str = ScriptRuntime.toString(objArr[TEST]);
        }
        double toInteger = (this.re.flags & MATCH) != 0 ? ScriptRuntime.toInteger(this.lastIndex) : 0.0d;
        if (toInteger < 0.0d || ((double) str.length()) < toInteger) {
            this.lastIndex = Double.valueOf(0.0d);
            return null;
        }
        int[] iArr = new int[MATCH];
        iArr[TEST] = (int) toInteger;
        Object executeRegExp = executeRegExp(context, scriptable, impl, str, iArr, i);
        if ((this.re.flags & MATCH) != 0) {
            toInteger = (executeRegExp == null || executeRegExp == Undefined.instance) ? 0.0d : (double) iArr[TEST];
    ...
    private static boolean flatNIMatcher(REGlobalData rEGlobalData, int i, int i2, String str, int i3) {
        ...
        for (int i4 = TEST; i4 < i2; i4 += MATCH) {
            char c = cArr[i + i4];
    ...
    private static int getIndex(byte[] bArr, int i) {
        return ((bArr[i] & 255) << 8) | (bArr[i + MATCH] & 255);
    }
    ...
    public static void init(Context context, Scriptable scriptable, boolean z) {
        ...
        nativeRegExp.re = compileRE(context, BuildConfig.FLAVOR, null, debug);
        nativeRegExp.activatePrototypeMap(MAX_PROTOTYPE_ID);
        ...
        nativeRegExp.defineProperty("constructor", (Object) nativeRegExpCtor, (int) PREFIX);
        ...
        ScriptableObject.defineProperty(scriptable, "RegExp", nativeRegExpCtor, PREFIX);
    }

    private static boolean isControlLetter(char c) {
        return (('a' > c || c > 'z') && ('A' > c || c > 'Z')) ? debug : true;
    }

    static boolean isDigit(char c) {
        return ('0' > c || c > '9') ? debug : true;
    }
    ...
    private static boolean isWord(char c) {
        return (('a' > c || c > 'z') && !(('A' <= c && c <= 'Z') || isDigit(c) || c == '_')) ? debug : true;
    }
    ...
    private static boolean matchRegExp(REGlobalData rEGlobalData, RECompiled rECompiled, String str, int i, int i2, boolean z) {
        ...
        boolean z2 = (z || (rECompiled.flags & JSREG_MULTILINE) != 0) ? true : debug;
        ...
        while (i3 <= i2) {
            if (c >= '\u0000') {
                while (i3 != i2) {
                    ...
                    if (charAt != c && ((rEGlobalData.regexp.flags & PREFIX) == 0 || upcase(charAt) != upcase((char) c))) {
                        i3 += MATCH;
                    }
                }
                return debug;
            ...
            for (i3 = TEST; i3 < rECompiled.parenCount; i3 += MATCH) {
            ...
            if (c != ANCHOR_BOL || rEGlobalData.multiline) {
                i3 = (rEGlobalData.skipped + i) + MATCH;
            } else {
    ...
    private static boolean parseAlternative(CompilerState compilerState) {
        ...
        if (rENode2 == null) {
            compilerState.result = new RENode(REOP_SIMPLE_START);
        } else {
    ...
    private static boolean parseDisjunction(CompilerState compilerState) {
        ...
        if (i != cArr.length && cArr[i] == '|') {
            ...
            if (rENode.kid.op == REOP_FLAT && rENode.kid2.op == REOP_FLAT) {
                rENode.op = (compilerState.flags & PREFIX) == 0 ? REOP_ALTPREREQ : REOP_ALTPREREQi;
                rENode.chr = rENode.kid.chr;
                rENode.index = rENode.kid2.chr;
                compilerState.progLength += 13;
            } else if (rENode.kid.op == REOP_CLASS && rENode.kid.index < EnchantType.flintAndSteel && rENode.kid2.op == REOP_FLAT && (compilerState.flags & PREFIX) == 0) {
                rENode.op = REOP_ALTPREREQ2;
                rENode.chr = rENode.kid2.chr;
                rENode.index = rENode.kid.index;
                compilerState.progLength += 13;
            } else if (rENode.kid.op == REOP_FLAT && rENode.kid2.op == REOP_CLASS && rENode.kid2.index < EnchantType.flintAndSteel && (compilerState.flags & PREFIX) == 0) {
                rENode.op = REOP_ALTPREREQ2;
                rENode.chr = rENode.kid.chr;
    ...
    private static boolean parseTerm(CompilerState compilerState) {
        ...
        switch (c2) {
            case Token.GETELEM /*36*/:
                ...
                compilerState.progLength += MATCH;
                return true;
            case Token.NUMBER /*40*/:
                rENode = null;
                i2 = compilerState.cp;
                if (compilerState.cp + MATCH < compilerState.cpend && cArr[compilerState.cp] == '?') {
                    c3 = cArr[compilerState.cp + MATCH];
                    if (c3 == '=' || c3 == '!' || c3 == ':') {
                        compilerState.cp += PREFIX;
                        if (c3 == '=') {
                            rENode = new RENode(REOP_ASSERT);
                            compilerState.progLength += JSREG_MULTILINE;
                        } else if (c3 == '!') {
                            rENode = new RENode(REOP_ASSERT_NOT);
                            compilerState.progLength += JSREG_MULTILINE;
                        }
                        compilerState.parenNesting += MATCH;
                        if (parseDisjunction(compilerState)) {
                            ...
                            reportError("msg.unterm.paren", BuildConfig.FLAVOR);
                            return debug;
                        }
                        return debug;
                    }
                ...
                compilerState.progLength += MAX_PROTOTYPE_ID;
                i2 = compilerState.parenCount;
                compilerState.parenCount = i2 + MATCH;
                ...
                return debug;
            case Token.STRING /*41*/:
                reportError("msg.re.unmatched.right.paren", BuildConfig.FLAVOR);
                return debug;
            case Token.NULL /*42*/:
            case Token.THIS /*43*/:
            ...
            case Token.FIRST_ASSIGN /*91*/:
                ...
                while (compilerState.cp != compilerState.cpend) {
                    if (cArr[compilerState.cp] == '\\') {
                        ...
                        compilerState.classCount = i4 + MATCH;
                        rENode.index = i4;
                        rENode = compilerState.result;
                        i4 = compilerState.cp;
                        compilerState.cp = i4 + MATCH;
                ...
                reportError("msg.unterm.class", BuildConfig.FLAVOR);
                return debug;
            case Token.ASSIGN_BITOR /*92*/:
                if (compilerState.cp < compilerState.cpend) {
                    i4 = compilerState.cp;
                    ...
                    switch (c2) {
                        case Token.REGEXP /*48*/:
                            reportWarning(compilerState.cx, "msg.bad.backref", BuildConfig.FLAVOR);
                            i = TEST;
                    ...
                    while (i4 < i && compilerState.cp < compilerState.cpend) {
                        ...
                        compilerState.cp = decimalValue + MATCH;
                        decimalValue = Kit.xDigitToInt(cArr[decimalValue], i2);
                        if (decimalValue < 0) {
                            compilerState.cp -= i4 + PREFIX;
                            i = compilerState.cp;
                            compilerState.cp = i + MATCH;
                            i = cArr[i];
                            doFlat(compilerState, (char) i);
                    ...
                    reportError("msg.trail.backslash", BuildConfig.FLAVOR);
                    return debug;
                }
                break;
        ...
        switch (cArr[compilerState.cp]) {
            ...
            case Token.THIS /*43*/:
                compilerState.result = new RENode(REOP_QUANT);
                compilerState.result.min = MATCH;
                compilerState.result.max = -1;
                compilerState.progLength += 8;
                z = true;
                break;
            case Token.ENUM_ID /*63*/:
                compilerState.result = new RENode(REOP_QUANT);
                compilerState.result.min = TEST;
                compilerState.result.max = MATCH;
                compilerState.progLength += 8;
                z = true;
                break;
            case Token.VAR /*123*/:
                decimalValue = compilerState.cp;
                i = compilerState.cp + MATCH;
                compilerState.cp = i;
                if (i < cArr.length) {
        ...
        if (compilerState.cp >= compilerState.cpend || cArr[compilerState.cp] != '?') {
            ...
            compilerState.cp += MATCH;
            compilerState.result.greedy = debug;
        }
        return true;
    }
    ...
    private static boolean reopIsSimple(int i) {
        return (i < MATCH || i > 23) ? debug : true;
    }
    ...
    private static int toASCIIHexDigit(int i) {
        ...
        int i2 = i | 32;
        return (97 > i2 || i2 > Token.LAST_ASSIGN) ? -1 : (i2 - 97) + 10;
    }
    ...
    public Object call(Context context, Scriptable scriptable, Scriptable scriptable2, Object[] objArr) {
        return execSub(context, scriptable, objArr, MATCH);
    }

    Scriptable compile(Context context, Scriptable scriptable, Object[] objArr) {
        if (objArr.length <= 0 || !(objArr[TEST] instanceof NativeRegExp)) {
            String escapeRegExp = (objArr.length == 0 || (objArr[TEST] instanceof Undefined)) ? BuildConfig.FLAVOR : escapeRegExp(objArr[TEST]);
            String scriptRuntime = (objArr.length <= MATCH || objArr[MATCH] == Undefined.instance) ? null : ScriptRuntime.toString(objArr[MATCH]);
            this.re = compileRE(context, escapeRegExp, scriptRuntime, debug);
            this.lastIndex = Double.valueOf(0.0d);
        } else if (objArr.length <= MATCH || objArr[MATCH] == Undefined.instance) {
            NativeRegExp nativeRegExp = (NativeRegExp) objArr[TEST];
    ...
    public Scriptable construct(Context context, Scriptable scriptable, Object[] objArr) {
        return (Scriptable) execSub(context, scriptable, objArr, MATCH);
    }
    ...
    public Object execIdCall(IdFunctionObject idFunctionObject, Context context, Scriptable scriptable, Scriptable scriptable2, Object[] objArr) {
        ...
        switch (methodId) {
            ...
            case JSREG_MULTILINE /*4*/:
                return realThis(scriptable2, idFunctionObject).execSub(context, scriptable, objArr, MATCH);
            case MAX_INSTANCE_ID /*5*/:
                return Boolean.TRUE.equals(realThis(scriptable2, idFunctionObject).execSub(context, scriptable, objArr, TEST)) ? Boolean.TRUE : Boolean.FALSE;
            case MAX_PROTOTYPE_ID /*6*/:
                return realThis(scriptable2, idFunctionObject).execSub(context, scriptable, objArr, PREFIX);
            default:
    ...
    Object executeRegExp(Context context, Scriptable scriptable, RegExpImpl regExpImpl, String str, int[] iArr, int i) {
        ...
        if (!matchRegExp(rEGlobalData, this.re, str, i2, length, regExpImpl.multiline)) {
            return i != PREFIX ? null : Undefined.instance;
        } else {
            Object obj;
            ...
            if (i == 0) {
                ...
                obj = context.newArray(scriptable, (int) TEST);
                scriptable2 = (Scriptable) obj;
                scriptable2.put((int) TEST, scriptable2, str.substring(i5, i5 + i4));
            }
            if (this.re.parenCount == 0) {
                ...
                for (int i6 = TEST; i6 < this.re.parenCount; i6 += MATCH) {
                    ...
                    if (parensIndex != -1) {
                        ...
                        if (i != 0) {
                            scriptable2.put(i6 + MATCH, scriptable2, subString.toString());
                        }
                    } else if (i != 0) {
                        scriptable2.put(i6 + MATCH, scriptable2, Undefined.instance);
                    }
                }
            ...
            if (context.getLanguageVersion() == Token.FOR) {
                ...
                regExpImpl.leftContext.index = TEST;
    ...
    protected Object getInstanceIdValue(int i) {
        ...
        switch (i) {
            ...
            case JSREG_MULTILINE /*4*/:
                if ((this.re.flags & PREFIX) == 0) {
                    z = debug;
                }
                return ScriptRuntime.wrapBoolean(z);
            case MAX_INSTANCE_ID /*5*/:
                if ((this.re.flags & JSREG_MULTILINE) == 0) {
                    z = debug;
                }
                return ScriptRuntime.wrapBoolean(z);
            default:
